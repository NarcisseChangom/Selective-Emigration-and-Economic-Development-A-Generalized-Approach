...
tic
% Set directory 
setpref('MyApp', 'mydir', 'U:\CHANGOM\CDDM2021_Completefile\CHAP1\Replication-ier\matlab');
mydir = getpref('MyApp', 'mydir');
cd(mydir)

%% Benchmark (Master program) 
run('dataprepa_IER.m'); % Load data 

run('Parameters_Bench_IER.m'); % Set parameter values 
run('Monte_carlo_like_calibration_z_IER.m'); % Identifies the optimal z by level of development 

run('Migration_costs_and_model_inversion_IER.m'); % Calibrate the migration costs and invert the model to recover unobservable location characteristics 

run('InitEqlm_IER.m'); % Prepare the initial equilibrium 


run('Partial_Eqlm_nomig_IER.m'); % Simulate a no-migration counterfactual under exogeneous wages 
...
run('Partial_Eqlm_noselec_IER.m'); % Simulate neutral selection counterfactual under exogeneous wages 
...
run('GenEqlm_SODC_nomig_IER.m'); % Simulate a no-migration counterfactual under endogeneous wages at origin and exogeneous wages at destination (SODC hypothesis)
run('GenEqlm_SODC_noselec_IER.m'); % Simulate a neutral selection counterfactual under endogeneous wages at origin and exogeneous wages at destination (SODC hypothesis)


run('GenEqlm_World_nomig_IER.m');  % Simulate a no-migration counterfactual under endogeneous wages at origin and destination (World General Equilibrium)
run('GenEqlm_World_noselec_IER.m');  % Simulate a neutral selection counterfactual under endogeneous wages at origin and destination (World General Equilibrium)
...
writetable(AVG_NIRb, 'output\AVG_NIR_Bench_25.csv', 'Delimiter',',' ,'QuoteStrings', true)


%% Channels 
writematrix(cc_channels, 'output\cc_channelsNM_b_25.csv', 'Delimiter',',' ,'QuoteStrings', true)
writematrix(cc_channels_NS, 'output\cc_channelsNS_b_25.csv', 'Delimiter',',' ,'QuoteStrings', true)
...
% Export to CSV 
writetable(NetIncomeResponse, 'output\Benchmark_IER_25.csv', 'Delimiter',',' ,'QuoteStrings', true)
...
% Export to CSV 
writetable(WAGES, 'output\WAGES_25.csv', 'Delimiter',',' ,'QuoteStrings', true)
...
writetable(PuEduc, 'output\GstarValidation_25.csv', 'Delimiter',',' ,'QuoteStrings', true)
...
HumanCap = renamevars(HumanCap,["isoo","Var2","Var3","Var4","Var5","Var6"...
                               ...
                               ,"UMIC","HIC","mH","mL","HnNM","HnPE","HnGE"]);
% Export to CSV 
writetable(HumanCap, 'output\HumanCap_Bench_25.csv', 'Delimiter',',' ,'QuoteStrings', true)
...
writetable(THEIL_Bench, 'output\THEIL_Bench_25.csv', 'Delimiter',',' ,'QuoteStrings', true)
...
run('Master_SelecMig_IER_Sensitivity2025');
...
writetable(THEIL_IER, 'output\THEIL_IER_25.csv', 'Delimiter',',' ,'QuoteStrings', true)
...
writetable(AVG_NIR, 'output\AVG_NIR_25.csv', 'Delimiter',',' ,'QuoteStrings', true)
